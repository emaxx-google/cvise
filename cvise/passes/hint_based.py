...
from cvise.passes.abstract import AbstractPass, BinaryState
from cvise.utils.hint import apply_hints, load_hints, store_hints

HINTS_FILE_NAME = 'hints.jsonl'
...
class HintState:
    ...
    def create(hint_count: int, hints_file_path: Path) -> HintState:
        binary_state = BinaryState(hint_count)
...
class HintBasedPass(AbstractPass, metaclass=ABCMeta):
    ...
    def transform(self, test_case, state, process_event_notifier):
        hints = load_hints(state.hints_file_path, state.binary_state.begin(), state.binary_state.end())
