...
def init(mode: Mode) -> None:
    ...
    os.write(
        sys.stderr.fileno(),
        f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor.init: initialized handlers\n'.encode(),
    )
...
def _on_signal(signum: int, frame) -> None:
    os.write(
        sys.stderr.fileno(),
        f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor._on_signal: signum={signum}\n'.encode(),
    )
    ...
    os.write(
        sys.stderr.fileno(),
        f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor._on_signal: doing future.done\n'.encode(),
    )
    # Set the exception on the future, unless it's already done. We don't use done() because it'd be potentially racy.
    with contextlib.suppress(concurrent.futures.InvalidStateError):
        os.write(
            sys.stderr.fileno(),
            f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor._on_signal: doing future.set_exception\n'.encode(),
        )
        _future.set_exception(exception)
        os.write(
            sys.stderr.fileno(),
            f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor._on_signal: did future.set_exception\n'.encode(),
        )
    ...
    if _mode == Mode.RAISE_EXCEPTION and not _can_raise_in_frame(frame):
        os.write(
            sys.stderr.fileno(),
            f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor._on_signal: exit due to !_can_raise_in_frame\n'.encode(),
        )
    ...
    if _mode == Mode.RAISE_EXCEPTION and signum == signal.SIGINT:
        os.write(
            sys.stderr.fileno(),
            f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor._on_signal: doing default_int_handler\n'.encode(),
        )
...
def _trigger_signal_action(signum: int, exception: BaseException) -> None:
    if _mode == Mode.QUICK_EXIT:
        os.write(
            sys.stderr.fileno(),
            f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor._trigger_signal_action: doing _exit\n'.encode(),
        )
        os._exit(1)
    else:
        os.write(
            sys.stderr.fileno(),
            f'[{datetime.now(UTC).isoformat(sep=" ", timespec="milliseconds")}] sigmonitor._trigger_signal_action: doing raise {type(exception).__name__}\n'.encode(),
        )
        raise exception
